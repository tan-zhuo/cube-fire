...
2. **邀请**：点左上角「对战设置」，把 **6 位房间码**告诉朋友（微信 / QQ 均可）
3. **加入**：朋友打开同一个页面 → 「加入房间」→ 输入房间码 → 自动连接进入游戏

房间码依赖公共信令云（PeerJS）自动完成 WebRTC 握手，仅握手瞬间联网，
...
| `lan.js` | 房间码信令（PeerJS）、WebRTC 手动信令、传输层封装、大厅交互、首页动效 |
